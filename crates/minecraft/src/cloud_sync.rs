...
impl CloudSync {
    ...
    pub fn share_instance(&self, instance_path: &Path) -> Result<String, Box<dyn std::error::Error>> {
        ...
        let response = self.client.post(&format!("{}/api/share", self.endpoint))
            .body(file_content)
            .send()?;
    ...
    pub fn import_instance(&self, url: &str, dest_path: &Path) -> Result<(), Box<dyn std::error::Error>> {
